...
CCurveView::CCurveView()
{
	...
	m_PenRCurve.CreatePen(PS_SOLID,2,RGB(0,255,0));
	m_PenOCurve.CreatePen(PS_SOLID,2,RGB(0,0,255));
	m_PenECurve.CreatePen(PS_SOLID,2,RGB(255,0,0));
...
CCurveView::~CCurveView()
{
	if(!m_TimeList.empty())
		m_TimeList.clear();
	if(!m_RefValueList.empty())
		m_RefValueList.clear();
	if(!m_OutputValueList.empty())
		m_OutputValueList.clear();
    if(!m_ErrorValueList.empty())
		m_ErrorValueList.clear();
}

CUniSensDoc* CCurveView::GetDocument() // non-debug version is inline
{
...
void CCurveView::DrawTitle(CDC *pDC)
{
	CDC dcMem;
	BITMAP bm;
	CBitmap *pBmp;
...
void CCurveView::SetTitle(CString strTitle)
{	
	m_strTitle = strTitle;
	CClientDC dc(this); 
	Invalidate();
...
void CCurveView::DrawValuePanel(CDC *pDC)
{
	CFont *pOldFont;
	CPen *pOldPen;
	...
	pDC->TextOut(m_rectValue.right+45 ,m_rectValue.top+2,m_strValueName1);
	pDC->TextOut(m_rectValue1.right+45,m_rectValue.top+2,m_strValueName2);

	pDC->TextOut(m_rectValuePanel.right-50,m_rectValue2.top+2,m_strUnitName);

	pDC->SelectObject(pOldFont);

}

void CCurveView::DrawValue(CDC *pDC)
{
	pDC->FillRect(&m_rectValue,&m_CurveBrush);
	pDC->FillRect(&m_rectValue1,&m_CurveBrush);
	pDC->FillRect(&m_rectValue2,&m_CurveBrush);

	CFont *pOldFont;
	pDC->DrawEdge(&m_rectValue,EDGE_SUNKEN,BF_RECT);
	pDC->DrawEdge(&m_rectValue1,EDGE_SUNKEN,BF_RECT);
	pDC->DrawEdge(&m_rectValue2,EDGE_SUNKEN,BF_RECT);

	pOldFont = pDC->SelectObject(&m_FontValue);
	CString str;

	str.Format("%.8f",m_dValue);
...
void CCurveView::SetValue(double dValue, double dValue1, double dValue2)
{
	CClientDC dc(this);
	m_dValue  = dValue;
	m_dValue1 = dValue1;
...
void CCurveView::SetDimT(double newDimT)
{
	if(m_dDimT==newDimT)
		return;
	else
...
void CCurveView::SetDimy(double newDimY)
{
...
void CCurveView::SetBase(int nNewABase,int nRange)
{
...
void CCurveView::DrawCurvePanel(CDC *pDC)
{
	CPen *pOldPen;
...
void CCurveView::DrawGrid(CDC *pDC)
{
	CPen *pOldPen;
...
void CCurveView::DrawTrack(CDC *pDC,BOOL bPressed)
{
	CPen *pOldPen;
...
void CCurveView::DrawAxis(CDC *pDC)
{
	CFont *pOldFont;
	int i,nPosY;
	CString str;
...
void CCurveView::DrawArrow(CDC *pDC,int nIndex,BOOL bLighted)
{
...
void CCurveView::DrawCurve(CDC *pDC)    //绘制曲线的函数
{
	m_bDrawing = TRUE;
	int nSize = m_TimeList.size();
    //int nSize = m_ValueList.size();
	POINT *pPointArray =(POINT *) new POINT[nSize];
	POINT *pPointArray1 =(POINT *) new POINT[nSize];
	POINT *pPointArray2 =(POINT *) new POINT[nSize];
	int i;
	
	std::list<double>::iterator time_it = m_TimeList.begin();
	std::list<double>::iterator value_it = m_RefValueList.begin();
	std::list<double>::iterator value_it1 = m_OutputValueList.begin();//==
	std::list<double>::iterator value_it2 = m_ErrorValueList.begin();//==
	...
	for(i=0;i<nSize;i++)
	{
		...
		pPointArray1[i].x = ConvertTimeToAxisX(*time_it-*time_begin);//==
		pPointArray1[i].y = ConvertValueToAxisY(*value_it1);//==
		pPointArray2[i].x = ConvertTimeToAxisX(*time_it-*time_begin);//==
		pPointArray2[i].y = ConvertValueToAxisY(*value_it2);//==
		time_it++;
		value_it++;
		value_it1++;//==
		value_it2++;//==
	...
	CPen *pOldPen = pDC->SelectObject(&m_PenOCurve);//==
	pDC->SelectClipRgn(&m_rgnCurve);//==
	if(isCheckOutput)
		pDC->Polyline(pPointArray1,nSize);//==

	pDC->SelectObject(&m_PenECurve);//==
	if(isCheckError)
		pDC->Polyline(pPointArray2,nSize);//==

	pDC->SelectObject(&m_PenRCurve);
//	pDC->SelectClipRgn(&m_rgnCurve);
	if(isCheckOrder)
...
void CCurveView::AddValue(double dTime,double dValue,double dValue1,double dValue2)
{
	if(m_bDrawing)
		return;

	m_TimeList.push_back(dTime);
	m_RefValueList.push_back(dValue);
    m_OutputValueList.push_back(dValue1);
    m_ErrorValueList.push_back(dValue2);
	...
	while(dEndTime-dStartTime>0.5*m_dDimT)
	{
		m_TimeList.pop_front();
		m_RefValueList.pop_front();
		m_ErrorValueList.pop_front();
		m_OutputValueList.pop_front();
...
void CCurveView::AddvalueTime(double dTime)
{
	if(m_bDrawing)
		return;
	m_TimeList.push_back(dTime);
...
void CCurveView::AddvalueData(double dValue)
{
	if(m_bDrawing)
		return;
    m_RefValueList.push_back(dValue);
...
void CCurveView::AddOvalueData(double dValue)//==
{
    if(m_bDrawing)//==
		return;   //==
    m_OutputValueList.push_back(dValue);//==
}

void CCurveView::AddEvalueData(double dValue)
{
	if(m_bDrawing)//==
		return;   //==
    m_ErrorValueList.push_back(dValue);//==
}

inline int CCurveView::ConvertTimeToAxisX(double dTime)
{
	int nReturn;
	nReturn = m_rectCurve.left+dTime/(0.5*m_dDimT)*m_rectCurve.Width();
	
	return nReturn;
}
inline int CCurveView::ConvertValueToAxisY(double dValue)
{
	int nReturn;
	int nCenterY  = m_rectCurve.top+(int)(m_nBase/(double)m_nRange*m_rectCurve.Height());
...
void CCurveView::SetValueString(CString strTitle,CString strUnit)
{
	m_strValueName = strTitle;
	m_strUnitName = strUnit;
	Invalidate();
}
void CCurveView::Clear()
{
	m_TimeList.clear();
	m_RefValueList.clear();
	UpdateCurve();
...
void CCurveView::PlayFromFile()
{
	...
	this->m_RefValueList.clear();
	this->m_OutputValueList.clear();
	this->m_ErrorValueList.clear();
	this->Invalidate();
	TRACE("Start timer\n");
	SetTimer(0,20,NULL);
	//SetTimer(2,50,NULL);  
