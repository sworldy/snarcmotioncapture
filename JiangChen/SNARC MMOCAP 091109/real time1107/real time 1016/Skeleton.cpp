...
void BoneSetFrameTest(t_Bone *bone,int frame)
{
	...
	if (bone->primChannel != NULL)
	{
		offset = (float *)(bone->primChannel + (4 * frame));
		...
		switch (bone->primChanType)
		{
			// TYPE_SRT HAS 9 FLOATS IN TXYZ,RXYZ,SXYZ ORDER
		case 11:
			bone->axisangle.theta = offset[0];
            bone->axisangle.ax = offset[1];
			bone->axisangle.ay = offset[2];
			bone->axisangle.az = offset[3];
		//	sf_file << offset[0] << "\t" << offset[1] << "\t"
		//	<< offset[2] << "\t" << offset[3] << endl;
			break;
		}
	}
}
...
//// BoneAdvanceFrame /////////////////////////////////////////////////////////////////
/*void BoneAdvanceFrameTest(t_Bone *bone,int direction,BOOL doChildren)
{
	///// Local Variables ///////////////////////////////////////////////////////////
  // bone->primCurFrame += direction;
   if (bone->primCurFrame >=29)
	   bone->primCurFrame = 0;
   if (bone->primCurFrame < 0)
	   bone->primCurFrame += bone->primFrameCount;
   BoneSetFrameTest(bone,(int)bone->primCurFrame);
   bone->primCurFrame += direction;
}*/