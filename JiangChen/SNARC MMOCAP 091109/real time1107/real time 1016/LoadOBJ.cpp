...
void HandleFaceTest(CStringArray *words,t_faceIndex *face)
{
	/// Local Variables ///////////////////////////////////////////////////////////
	int loop,loopcnt;
	CString temp;
...
void ModifyPointofHandleFaceTest (CStringArray *words,t_faceIndex *face)
{
	/// Local Variables ///////////////////////////////////////////////////////////
	int loop,loopcnt;
	CString temp;
	...
	{
		temp = words->GetAt(loop);			// GRAB THE NEXT WORD
		face->v[loop - 1] = atoi(temp);		// STORE OFF THE INDEX FOR THE VERTEX
	}
}
///// HandleFace //////////////////////////////////////////////////////////////
...
/// Local Variables ///////////////////////////////////////////////////////////
	...
	long vPos = 0, nPos = 0, tPos = 0, fPos = 0, pPos = 0;
...
///////////////////////////////////////////////////////////////////////////////
	...
	if (fp != NULL)
	{
		...
		if (vCnt > 0)
		{
			...
			//visual->vertexCnt = vCnt*23;//add by wz
		...
		if (vCnt > 0)
		{
				...
				if (cnt > 0)
				{
					...
					if (temp.GetLength() > 0)
					{
						...
						else if (temp[0] == 'f')			// f v/t/n v/t/n v/t/n	FACE LINE
						{
							...
							HandleFaceTest(&words,&face[fPos]);

                            for (int n=0;n< words.GetSize()-1;n++)
                            {
								 recod_ver_id[num] = face[fPos].v[n];
								 num++;
                            }
						...
						//{
						//	//add by wz
						//	ModifyPointofHandleFaceTest(&words,&face[pPos]);
						//	pPos++;
						//}
						else if (temp == "mtllib")  // HANDLE THE MATERIAL LIBRARY
						{
			...
			if (nCnt > 0)
			{
				if (tCnt > 0)
				{
					visual->dataFormat = GL_T2F_N3F_V3F;
					visual->vSize = 8;					// 2 texture, 3 normal, 3 vertex
					visual->vertexData = (float *)malloc(sizeof(float) * visual->vSize * temp_sum);
					visual->deformData = (float *)malloc(sizeof(float) * visual->vSize * temp_sum);
					visual->CV_select = (BOOL *)malloc(sizeof(BOOL) * visual->vSize * temp_sum);
					visual->faceCnt = fPos;
				}
				else
				{
			...
			//visual->vertexCnt = visual->vPerFace * visual->faceCnt;	// Set the vertex count
			visual->vertexCnt = temp_sum;//add by wz
			...
			for (loop = 0; loop < fPos; loop++)
			{
				...
				for (loop2 = 0; loop2 < face[loop].ver_count; loop2++)
				{
					// ALL FACE INDICES ARE 1 BASED INSTEAD OF 0
					if (tCnt > 0)	// IF TEXTURE COORDS WRITE OUT THOSE
					{
					...
					*data++ = vertex[face[loop].v[loop2] - 1].z;
					...
					//visual->CV_select[(loop * visual->vPerFace) + loop2] = FALSE;
					visual->CV_select[temp_count + loop2] = FALSE;

				}
				    temp_count += face[loop].ver_count;
			}


			*counter = statThreeandFour(visual,face,cur_index);
