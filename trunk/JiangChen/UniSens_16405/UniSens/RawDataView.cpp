...
IMPLEMENT_DYNCREATE(CRawDataView, CView)


CRawDataView::CRawDataView()
{
	m_strTitle = _T("Acceleration");
	m_strValueName = _T("Acc X");
	m_strValueName1 = _T("Acc Y");
	m_strValueName2 = _T("Acc Z");
	m_strUnitName = _T("m/s2");
...
BEGIN_MESSAGE_MAP(CRawDataView, CView)
	//{{AFX_MSG_MAP(CRawDataView)
...
/////////////////////////////////////////////////////////////////////////////
// CRawDataView drawing
BOOL CRawDataView::OnEraseBkgnd(CDC* pDC) 
{
...
void CRawDataView::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: Add your message handler code here and/or call default
	...
	if(row < frames)
	{
		...
		acc_z = v->at(row).at(2); 
		row++;
		AddValue(dTime, acc_x, acc_y, acc_z);
		SetValue(acc_x, acc_y, acc_z);
		UpdateCurve();
	}
