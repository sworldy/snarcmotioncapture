...
QKF::QKF(double *gintial, double *minitial){
	...
	gravity_initial[0]=gintial[0];//加速度初值
	gravity_initial[1]=gintial[1];
	gravity_initial[2]=gintial[2];
	mag_intitial[0]=minitial[0];//磁初值
	mag_intitial[1]=minitial[1];
	mag_intitial[2]=minitial[2];
	angular_average[0]=gintial[3];//角速度均值
	angular_average[1]=gintial[4];
	angular_average[2]=gintial[5];
...
void QKF::Update(double *measurment){
    ...
    for (int j=3;j<6;j++)
	{
		measurment[j] = measurment[j] - angular_average[j-3];//角速度去漂移
	}
	for (i=0;i<9;i++)
	{
