...
bool Fusion::WholeBodyFusion(double QuatResult[][DIM_QUAT],double *DisResult)
{
	...
	for(i=0; i<SENSOR_NUM; i++)
	{
		...
		if(i == 3)
			Displacement->SetPara(1, Quaternions[i], Measure_Buf[i]); //Displacement in Pelvis(No.1)
		if(i == 9)
			Displacement->SetPara(17, Quaternions[i], Measure_Buf[i]);//Displacement in LFoot(No.17)
		if(i == 2)
			Displacement->SetPara(21, Quaternions[i], Measure_Buf[i]);//Displacement in RFoot(No.21)
		if (i == 7)
			Displacement->SetPara(15, Quaternions[i], Measure_Buf[i]); //Displacement in LThigh(No.15)
		if (i == 8)
			Displacement->SetPara(16, Quaternions[i], Measure_Buf[i]); //Displacement in LCalf(No.16)
		if (i == 0)
			Displacement->SetPara(19, Quaternions[i], Measure_Buf[i]); //Displacement in RThigh(No.19)
		if (i == 1)
			Displacement->SetPara(20, Quaternions[i], Measure_Buf[i]); //Displacement in RCalf(No.20)
	}
