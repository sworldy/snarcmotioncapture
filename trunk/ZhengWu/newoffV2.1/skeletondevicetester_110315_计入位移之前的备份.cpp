...
int sensor2bone[31] = {3,0,1,2,NAN,NAN,7,8,9,NAN,NAN,4,4,5,6,NAN,16,10,11,12,NAN,NAN,NAN,17,13,14,15,NAN,NAN,NAN};
bool Cleanup()
{
    if (WSACleanup()) 
	{
...
int main(int argc, char* argv[])
{
	...
	for (int j = 0; j < (int)dataLen; j++)
	{
		...
		/*Quat_Dis->Quaternion_Augment(Quat_ResAug,Quat_Result);
		for (int i = 0; i<2; i++)
		{
			for (int k = 0; k < 4; k++)
				Quat_Result[16+i][k] = Quat_ResAug[i][k];
		}*/
		...
		/*if (j != 0)
		{
			for (int i = 0; i < SENSOR_NUM+2; i++)
			{
				...
				{
					dtmp3[k] = -1 * dtmp[i][k];
					dtmp4[k] = Quat_Result[i][k];
				}
				dtmp3[3] = -1 * dtmp3[3];
				Quat_Dis->Quaternion_Multiple(dtmp3,dtmp4);
				for (int k = 0; k < 4; k++)
				{
					dtmp[i][k] = Quat_Result[i][k];
					Quat_Result[i][k] = dtmp3[k];
				}
			}
		}
		else
