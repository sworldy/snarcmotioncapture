...
void UpdateInformation(sDataBuffer& mDataBuffer, int ID,FBMatrix TransMat)
{
	...
	case 3:
		...
		for (int i = 0; i < 3; i++)
		{
			mDataBuffer.mChannel[ID+1].mR[i] = mDataBuffer.mChannel[ID].mR[i];
			mDataBuffer.mChannel[ID+1].mT[i] = mDataBuffer.mChannel[ID].mT[i];
			mDataBuffer.mChannel[ID+2].mR[i] = mDataBuffer.mChannel[ID].mR[i];
			mDataBuffer.mChannel[ID+2].mT[i] = mDataBuffer.mChannel[ID].mT[i];
		}
		break;
	case 4:
		for (int i = 0; i < 3; i++)
	...
	case 19:
		...
		for (int i = 0; i < 3; i++)
		{
			...
			mDataBuffer.mChannel[ID+2].mR[i] = mDataBuffer.mChannel[ID].mR[i];
			mDataBuffer.mChannel[ID+2].mT[i] = mDataBuffer.mChannel[ID].mT[i];
			mDataBuffer.mChannel[ID+3].mR[i] = mDataBuffer.mChannel[ID].mR[i];
			mDataBuffer.mChannel[ID+3].mT[i] = mDataBuffer.mChannel[ID].mT[i];
